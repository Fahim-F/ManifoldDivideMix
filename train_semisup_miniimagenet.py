...
from datasets.miniimagenet_dataset import MiniImagenet
...
def get_arguments():
    parser = argparse.ArgumentParser(
        description="SemiSupervised training of a pretrained model on Miniimagenet"
    )
    # Data
    parser.add_argument("--data-dir", default= "/dataset/mini-imagenet/", type=str, help="path to dataset")
    ...
    parser.add_argument('--lambda_u', default=100, type=float, help='weight for unsupervised loss') 
    ...
    parser.add_argument('--lambda_ue', default=100, type=float, help='weight for contrastive loss applied on embeddings') 
...
def main():
    ...
    transform_weak_C100 = transforms.Compose(
                [
                    ...
                    transforms.ToTensor(),
                    transforms.Normalize((0.4728, 0.4487, 0.4031), (0.2744, 0.2663 , 0.2806)),
                ]
            )
    ...
    train_dataset = MiniImagenet(ind_ratio=args.noise_ratio, root_dir=args.data_dir, transform=transform_weak_C100, color=args.noise_type, mode="all_sup")                
    val_dataset = MiniImagenet(ind_ratio=args.noise_ratio, root_dir=args.data_dir, transform=val_transforms, color=args.noise_type, mode="all_sup" )                
    test_dataset = MiniImagenet(ind_ratio=args.noise_ratio, root_dir=args.data_dir, transform=val_transforms,  mode="test")                
    kwargs = dict(
    ...
    removal_ratio = int(np.ceil(args.remove_ood*len(val_dataset)))
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch < args.warmup_epoch:
            ...
            if epoch == args.warmup_epoch or ( epoch % args.interval == 0 and args.clustering):
                removal_ratio = removal_ratio + int(np.ceil(args.remove_ood*len(val_dataset)))
                ood_index = find_outliers(val_loader, model, args.gpu, removal_ratio)
                mask_ood = 1 - ood_index
            
                pred, prob = seperate_lul_sets(args, val_loader, model, epoch)
                
                labeled_dataset = MiniImagenet( ind_ratio=args.noise_ratio,root_dir=args.data_dir, transform=transform_weak_C100, mode="labeled", ood=mask_ood.cpu().numpy(), pred=pred, probability=prob, transform_st=transform_strong_C100, color=args.noise_type)
                unlabeled_dataset = MiniImagenet(ind_ratio=args.noise_ratio,root_dir=args.data_dir, transform=transform_weak_C100, mode="unlabeled",  ood=mask_ood.cpu().numpy(), pred=pred, transform_st=transform_strong_C100, color=args.noise_type)                    
            ...
            for step, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x) in enumerate(labeled_trainloader, start=epoch * len(labeled_trainloader)):      
                ...
                with torch.no_grad():
                    ...
                    max_probs, pseudo_label = torch.max(targets_u, dim=-1)
