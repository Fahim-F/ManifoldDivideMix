'''
    ...
    Original Repository: https://github.com/hitcszx/ALFs/blob/master/lnl/losses.py 
'''
...
class SCELoss(nn.Module):
    def __init__(self, out_type='mean', num_classes=10, a=1, b=1):
    ...
    def forward(self, pred, labels, onehot=False):
        if onehot:
            ...
            label_one_hot = torch.clamp(label_one_hot, min=1e-4, max=1.0)
            ce = -1* torch.sum(F.log_softmax(pred, dim=1) * labels, dim=1)
            # RCE
            pred = F.softmax(pred, dim=1)
            pred = torch.clamp(pred, min=eps, max=1.0)
            labels = torch.clamp(labels, min=eps, max=1.0)
            rce = -1* torch.sum(F.log_softmax(labels, dim=1) * pred, dim=1)
        if (self.out_type == 'mean'):
            loss = self.a * ce.mean() + self.b * rce.mean()
        else:
            loss = self.a * ce + self.b * rce
        return loss
class RCELoss_mean(nn.Module):
    def __init__(self, num_classes=2, scale=1.0):
        super(RCELoss_mean, self).__init__()
        self.num_classes = num_classes
        self.scale = scale

    def forward(self, pred, labels, onehot=True):
        pred = F.softmax(pred, dim=1)
        pred = torch.clamp(pred, min=eps, max=1.0)
        if onehot:
            label_one_hot = F.one_hot(labels, self.num_classes).float().to(pred.device)
...
class NCELoss_mean(nn.Module):
    def __init__(self, num_classes, scale=1.0):
        super(NCELoss_mean, self).__init__()
        self.num_classes = num_classes
        self.scale = scale
...
class MAELoss(nn.Module):
    def __init__(self, num_classes=2, scale=2.0):
...
class NGCELoss(nn.Module):
    def __init__(self, num_classes=2, q=0.7, scale=1.0):
...
class AGCELoss(nn.Module):
    def __init__(self, num_classes=2, a=1, q=2, eps=eps, scale=1.):
...
class AUELoss(nn.Module):
    def __init__(self, num_classes=2, a=1.5, q=0.9, eps=eps, scale=1.0):
...
class ANormLoss(nn.Module):
    def __init__(self, num_classes=2, a=1.5, p=0.9, scale=1.0):
...
class AExpLoss(torch.nn.Module):
    def __init__(self, num_classes=2, a=3, scale=1.0):
...
class NCEandRCE_mean(nn.Module):
    def __init__(self, alpha=1., beta=1., num_classes=10):
        super(NCEandRCE_mean, self).__init__()
        self.num_classes = num_classes
        self.nce = NCELoss_mean(num_classes=num_classes, scale=alpha)
        self.rce = RCELoss_mean(num_classes=num_classes, scale=beta)

    def forward(self, pred, labels, onehot=True):
        return self.nce(pred, labels, onehot) + self.rce(pred, labels, onehot)
...
class NLNL(torch.nn.Module):
    def __init__(self, train_loader, num_classes=2, ln_neg=1):
...
class NFLandRCE(torch.nn.Module):
    def __init__(self, size_average=True, alpha=1., beta=1., num_classes=10, gamma=0.5):
        super(NFLandRCE, self).__init__()
        self.num_classes = num_classes
        self.size_average = size_average
        self.nfl = NormalizedFocalLoss(gamma=gamma, num_classes=num_classes, size_average=size_average, scale=alpha)
        if size_average:
            self.rce = RCELoss(num_classes=num_classes, scale=beta)
        else:
            self.rce = RCELoss_mean(num_classes=num_classes,  scale=beta)
    ...
    def forward(self, pred, labels, onehot=True):
        return self.nfl(pred, labels) + self.rce(pred, labels, onehot=onehot)
...
class NCEandAGCE(torch.nn.Module):
    def __init__(self, alpha=1., beta = 1., num_classes=2, a=3, q=1.5):
...
class NCEandAUE(torch.nn.Module):
    def __init__(self, alpha=1., beta=1., num_classes=2, a=6, q=1.5):
...
class NCEandAEL(torch.nn.Module):
    def __init__(self, alpha=1., beta=4., num_classes=2, a=2.5):
...
class NFLandAGCE(torch.nn.Module):
    def __init__(self, alpha=1., beta = 1., num_classes=2, a=3, q=2):
...
class NFLandAUE(torch.nn.Module):
    def __init__(self, alpha=1., beta = 1., num_classes=2, a=1.5, q=0.9):
...
class NFLandAEL(torch.nn.Module):
    def __init__(self, alpha=1., beta = 1., num_classes=2, a=3):
...
class ANLandRCE(torch.nn.Module):
    def __init__(self, alpha=1., beta=1., num_classes=2, a=2, p=0.7):
